...
3、启动（是用python启动的，框架是java开发 bin是python写的，每台storm都要启动）
	...
	master节点
		run.sh
			...
			python bin/storm ui &    比如网站打开：master:8080/index.html页面
...
4、运行项目
	把java程序打成jar包：比如一个名称为chengxu工程打包成名称为dabaochengxu.jar    maven插件assembly
	python apache-storm-0.9.3/bin/storm jar ./dabaochengxu.jar chengxu.start remote
	python storm目录 jar ./打的jar包 项目名称.启动类 传参参数(给main函数args)
